...
unsigned long eFPartitionTotalSize(unsigned long perSize, unsigned long n){
	...
	*/
	int i = (perSize / 8) + 1;
	int b = (i * 8 + 4) * n + eFPartition_size;
	return b;
}		
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n){
	...
	*/
	...
	EEB* q = (p + eFPartition_size);
	int perSize1;
	perSize1 = ((perSize+7)/8)*8;
	for (; k < n; k++) {
		q->next_start = q + EEB_size + perSize1;
		q = q->next_start;
	}
	q->next_start = 0;
	return p;
...
unsigned long eFPartitionAlloc(unsigned long EFPHandler){
	...
	*/
	...
	EEB* q = p->firstFree;
	/*	EEB* r = (p + eFPartition_size);
	while (r->next_start != q)
		r = r->next_start;
	r->next_start = q;	*/		//使用链表无需维护
		p->firstFree = q->next_start;
		return q;
}
...
unsigned long eFPartitionFree(unsigned long EFPHandler,unsigned long mbStart){
	...
	*/	//释放的内存块未必是使用尾部
	...
	EEB* s, * t;
	t = 0;
	s = EFPHandler + eFPartition_size;
	EEB* m = mbStart;
	while (s->next_start != m->next_start) {
		t = s;
		s = s->next_start;
	}		//s=mbStart;t=mbStart前一项
	t->next_start = s->next_start;	//使用块t->next_start指向其后第二项
	m->next_start = p->firstFree;
	p->firstFree = m;		//m成为空闲首块，next_start指向原空闲首块
}