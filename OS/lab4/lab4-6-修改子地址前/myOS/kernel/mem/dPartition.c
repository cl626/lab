...
unsigned long dPartitionAllocFirstFit(unsigned long dp, unsigned long size){
	...
	*/
	...
	if (q->size > size) {
		m = q->nextStart;
		q->nextStart = q + EMB_size + size;	
		r = q->nextStart;
		r->nextStart = m;
		r->size = q->size- size-EMB_size;	//r->size+EMB_size=r->nextStart-r;
		q->size = size;
		p->firstFreeStart = r;
...
//检测相邻并在空闲块加入a
	...
	EMB* l, * r;
	unsigned long test;
	test = a + a->size + EMB_size;
	/*myPrintk(0x7, "a=%x\n", a);
	myPrintk(0x7, "a->size=%x\n", a->size);
	myPrintk(0x7, "EMB_size=%x\n", EMB_size);
	myPrintk(0x7, "EMB_size*8=%x\n", EMB_size*8);
	myPrintk(0x7, "a + EMB_size * 8=%x\n", (a+EMB_size*8));
	myPrintk(0x7, "a + (a->size + EMB_size) * 8=%x\n", test);*/
	do {
		if ((b + EMB_size + b->size) == a) {
			left = b; break;
		}		//有空闲块在要释放块左侧
		else if ((a + a->size + EMB_size) == b) {
			right = b;	break;
		}	//有空闲块在要释放块右侧，pMeminit处为此种
		//myPrintk(0x7, "%x %x %x %x %x \n", a, a->size, b,b->nextStart,right);
		b = b->nextStart;
	} while (b != 0);
	if (left==0 && right==0) {
