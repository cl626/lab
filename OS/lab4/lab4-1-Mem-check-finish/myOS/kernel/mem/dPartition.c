...
unsigned long dPartitionInit(unsigned long start, unsigned long totalSize){		//对齐
	...
	*/
	dPartition* p = start;
	p->size = totalSize;
	p->firstFreeStart = start + dPartition_size;
...
unsigned long dPartitionAllocFirstFit(unsigned long dp, unsigned long size){
	...
	*/
	...
	if (q->size >= size) {
		...
		r = q->nextStart;
		r->nextStart = m;
		r->size = q->size - 4 - size;
		q->size = size+4;
		p->firstFreeStart = r;
		myPrintk(0x7, "%d/%d/%d/%d\n", q, q->size, r, r->size);
		return r;
	}	
	else return 0;
}	//firstfit over!
...
//检测相邻并在空闲块加入a
	...
	EMB* l, * r;
	while (b->nextStart != 0) {
		if ((b + EMB_size + b->size) == m)	left = b;
		else if (m + m->size + EMB_size == b)	right = b;
		b = b->nextStart;
	}
	if (!left && !right) {
