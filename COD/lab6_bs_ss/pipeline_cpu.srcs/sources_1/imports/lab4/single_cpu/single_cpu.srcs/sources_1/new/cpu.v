...
.RegSrc	(RegSrc),	.MemWrite(Memwrite),	.Alusrc(Alusrc),    .Regwrite(Regwrite),	.ALUOp(ALUOp)	);    //无新增节点

Imm Immex(.command(ir),    .func3(ir[14:12]),   .imm(Imm));      //无新增节点
...
regfile regfile1(.we(ctrlw[18]), .wa(rdw),  .ra1(ir[19:15]),    .ra2(ir[24:20]),    .ra3(m_rf_addr[4:0]),
...
assign read2=(ctrl[4])?imm:((bfwd==0)?b:((bfwd==1)?y:((bfwd==2)?writedata:0)));       //ctrl[4]=alusrc  //写入不是mdr,imm写成Imm,纪念一下。。
assign b1=(bfwd==0)?b:((bfwd==1)?y:((bfwd==2)?writedata:0));
//alucontrol exemplification    //置于id周期
alucontrol alucontrolex(.ALUOp(ctrl[1:0]),	.func3(ire[14:12]),  .func7(ire[31:25]),	.alucontrol(alucontrol));      //新增func3
...
//single_data_mem	dm(y[9:2],bm[31:0],m_rf_addr,clk_cpu,ctrlm[12],readdata1,readdata2);		
single_data_mem	dm(y[9:2],bm[31:0],m_rf_addr,clk_cpu,iom,readdata1,readdata2);

wire[31:0] tem;         //第五周期。。
assign tem=(iow)?io_din:mdr;        //io_din/mdr/y      
assign writedata=(ctrlw[17:16]==2'b10)?(pcw+4):((ctrlw[17:16]==2'b01)?tem:((ctrlw[17:16]==2'b00)?yw:0));	 
//rd=目标寄存器地址;   yw=aluresult;   mdr=readdata;   
assign rf_data=rd3;
...
id_ex id_ex1(.clk_cpu(clk_cpu),  .fstall(fstall),   .dstall(dstall),    .dflush(dflush),    .eflush(eflush),  .pcd(pcd), 
 .rs1(rd1),    .rs2(rd2),    .Imm(Imm),    .ir(ir),  .jal(jal),  .branch(Branch),    .alusrc(Alusrc),   .memwrite(Memwrite),    
 .regwrite(Regwrite),    .afwd(afwd),    .bfwd(bfwd),    .aluop(ALUOp),  .regsrc(RegSrc),    .alucontrol(alucontrol),   .rda(ir[11:7]),       
...
hazard_branch hazard_branch1(.ir(ir),     .rd(rd),   .id_ex_regsrc(ctrl[17:16]),     .branch(ctrl[8]), 
   .zero(zero),    .jal(ctrl[9]),   .fstall(fstall),    .dstall(dstall),    .dflush(dflush),    .eflush(eflush) );    //第1个阶段测直接停顿
endmodule