...
initial begin pce<=32'h00003000;   a<=0;   b<=0;   imm<=0; ire<=0; ctrl<=0;  rd<=0;    id_ex_alucontrol<=0;    end    
always@(posedge clk_cpu)    begin 
   // if(eflush)  begin   pce<=pce;   a<=a;   b<=b;   imm<=imm;   ire<=ire;   ctrl<=0; rd<=rd; id_ex_alucontrol<=id_ex_alucontrol; end
  if(eflush)  begin  ctrl<=0;  end
    ...
    rd<=rda;   id_ex_alucontrol<=alucontrol;    ire<=ir;   end     
    end
endmodule
